...
Here is your improved slide, clearer, concise, and aligned with your instructions:

---

## Comparing Medical Procedures

Doctors compared two minimally invasive treatments for uterine fibroids to determine which:

1. **Is more effective at reducing fibroid size**.
2. **Is less invasive for patients**, measured by:
...
- Of course, that's **not valid statistics**!
- But this inspired a deeper question:

> How can someone systematically choose data points to remove in order to change a statistical conclusion?

---

## Background

- **Statistical conclusions** in this project are based on **Bayesian inference** (probabilistic ML).

- Bayesian methods infer unknown parameters from data using a **posterior distribution**, summarizing all uncertainty.

- Bayesian inference is widely used in science, medicine, and policy-making.

- There's an entire field, **Adversarial Machine Learning (AML)**, studying how data manipulations influence machine learning models.

- However, adversarial attacks targeting **Bayesian inference** specifically are not well understood. Our work addresses this gap.
...
- Bayesian inference is widely used across scientific disciplines.
...
- **Honest Bayesian** (defender, $D$) observes data $\mathbf{X}$.
- Goal: infer unknown parameters $\theta \in \mathbb{R}^d$.

Bayesian posterior contains all relevant inferential information:
$$
\pi(\theta | \mathbf{X}) \propto \exp\left(\sum_{i=1}^n \log \pi(X_i | \theta)\right) \pi(\theta)
$$
...
<div class="centered-figure">
  <img src="images/posterior_treatment_effect.jpeg" alt="My Image" style="max-width: 85%;">
</div>
...
$$
\pi_w(\theta | \mathbf{X}) \propto \exp\left(\sum_{i=1}^n w_i \log \pi(X_i|\theta)\right) \pi(\theta)
$$
...
Just removing $0.12 \%$ of the **strategically chosen** data points...


<div class="centered-figure">
  <img src="images/tainted_posterior_treatment_effect.jpeg" alt="My Image" style="max-width: 45%;">
...
- Solve the continuous relaxation first using **stochastic gradient-based optimization**. 
...
- Interestingly, we do not need a closed-form expression for neither the posterior nor the target distribution!
...
## Integer-Steps Coordinate Descent (ISCD)

- Start with initial feasible $w = \mathbf{1}$.
- At each iteration, consider feasible neighbors $w \pm e_i$.
- Choose neighbor with greatest estimated objective improvement:
$$
...
- Dominant computational cost: MCMC sampling to estimate gradients.
- Gradual changes in $w$ allow reuse of previous MCMC samples, reducing burn-in and computational overhead.
...
<div class="centered-figure">
  <img src="images/microcredit1.jpeg" alt="My Image" style="max-width: 58%;">
...
<div class="centered-figure">
  <img src="images/microcredit2.jpeg" alt="My Image" style="max-width: 58%;">
...
Infer parameters of a linear model for predicting housing prices from house characteristics:
...
::: {.column width="49%"}
![](images/boston1.jpeg){fig-align="center"}

KL Divergence vs Number of Manipulated Data Points
:::

::: {.column width="50%"}
![](images/boston2.jpeg){fig-align="center"}

Posterior mean of $\beta_{RM}$ vs Number of Manipulated Data Points
:::
...
<div class="centered-figure">
  <img src="images/boston3.jpeg" alt="My Image" style="max-width: 70%;">
...
::: {.column width="49%" .center}
![](images/boston4.jpeg){width=90%}

:::

::: {.column width="49%" .center}
![](images/boston5.jpeg){width=90%}
...
<div class="centered-figure">
  <img src="images/ubiña.jpg" alt="My Image" style="max-width: 70%;">
