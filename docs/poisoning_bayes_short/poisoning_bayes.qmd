...
1. Solve the continuous relaxation first using **projected stochastic gradient-based optimization**. 



2. Then, project the solution to the integer space.


---

## SGD for Continuous Relaxation
...
1. $\hat{g}$ is an unbiased estimate of the gradient objective function
2. $\gamma_t$ is a learning rate
3. $\Pi_{\mathcal{W}}$ is the projection operator onto the feasible set
...
We need an unbiased estimate $\hat{g}$:
